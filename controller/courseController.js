...
exports.getByCatId = (id, done) => {
    Course.findOne({ where: { catId: id } }).then((getCourse) => {
        if (getCourse) {
            db.query("select courses.*,chapters.courseId,chapters.files,users.fullname,subcategories.name as subcategory,categories.name as category, COUNT(chapters.courseId) as TotalChapter,GROUP_CONCAT(chapters.files) as lecture from courses,chapters,users,subcategories,categories where subcategories.id = courses.subcatId && categories.id = courses.catId " +
                "&&  courses.userID = users.id && courses.isDelete = 0 && categories.id = " + id + " && subcategories.catId = " + id + " && courses.catId = " + id + "&& chapters.courseId= courses.id GROUP BY chapters.courseId", { type: Sequelize.QueryTypes.SELECT })
                .then((cat) => {
                    done(null, cat)
                }).catch((err) => {
                    done(err)
...
exports.getCourseById = (courseId, done) => {
    db.query("select courses.*,chapters.courseId,chapters.files,users.fullname,categories.name as categoryname,subcategories.name as subcategoryname,COUNT(chapters.courseId) as TotalChapter,GROUP_CONCAT(chapters.files) as lecture from courses,chapters,users,categories,subcategories where  courses.userId=users.id && courses.isDelete = 0 && courses.catId = categories.id && courses.subcatId = subcategories.id && courses.id = " + courseId + "&& chapters.courseId= courses.id GROUP BY chapters.courseId", { type: Sequelize.QueryTypes.SELECT })
        .then((getCourse) => {
            if (getCourse) {
                done(null, getCourse);
            }
